...
MotorPID::MotorPID(uint8_t dir1, uint8_t dir2, uint8_t gpA, uint8_t gpB, uint8_t pwm) :
		MotorMgr(dir1, dir2, gpA, gpB, pwm){
	// NOP
}
...
float MotorPID::doPID(){
	...
	printf("delt %0.2f power (throttle) percent %0.2f\n", delt, t);
...
float MotorPID::pid (float &sp, float &pv, float &err, float &p, float &i, float &d){
	sp = xTargetRPM;
